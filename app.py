...
# ✅ NOVO (NÃO INTERFERE EM NADA EXISTENTE): Leads Status diário (coluna Q) + válidas
HIST_LEADS_STATUS_Q = os.path.join(DATA_DIR, "hist_leads_status_q.json")      # dd/mm/aaaa -> {"status":{...}, "validas":int}
...
# =========================================================
# SALVAR SNAPSHOT COMPARATIVO DO DIA (por DATA_BASE)
# =========================================================
...
# =========================================================
# TABELA COMPARATIVA (diferenças dia a dia)
# =========================================================
...
# =========================================================
# RECEITA LÍQUIDA (H1 + ASSIS E MOLLERKE)
# =========================================================
...
# =========================================================
# CONVERSÃO DO MÊS (DETALHAMENTO DIÁRIO)
# =========================================================
    ...
    with tabs[3]:
        ...
        def _compute_paid_max_before_month(target_mkey: str) -> Dict[str, float]:
            ...
            for mkey in months_sorted:
                ...
                if mkey == "12/2025":
                    faixa_nome_local, precos_local = FAIXAS[-1][1], FAIXAS[-1][2]
                else:
                    faixa_nome_local, precos_local = faixa_por_qtd(qtd_qual)
...
META_DIR = DATA_DIR
META_SUMMARY_PATH = os.path.join(META_DIR, "meta_c6_summary.json")  # resumo pequeno (ok no Firestore)
...
def _persist_summary(df_5cols: pd.DataFrame, files_sig: list):
    ...
    df["Mes"] = df["message_date_time"].dt.to_period("M").astype(str)  # YYYY-MM
...
# =========================================================
# ✅ NOVO BLOCO (APENAS O PEDIDO): LEADS — STATUS diário (coluna Q)
# + Indicações válidas (<=14 dias) no comparativo diário do Leads Status
# =========================================================
st.divider()
...
st.markdown(
    "<div style='color:#5b6b8c;font-weight:700'>"
    "Indicações válidas = (Data base da <b>coluna B</b>) − (Data/hora cadastro da <b>coluna M</b>) ≤ 14 dias."
    "</div>",
    unsafe_allow_html=True
)

up_leads_status = st.file_uploader(
    "Importar Leads (Status diário) — o STATUS deve estar na coluna Q (.xlsx)",
    type=["xlsx"],
    key="leads_status_q_upload"
)

def _leads_status_get_data_base_from_col_b(df: pd.DataFrame) -> Optional[dt.date]:
    # coluna B = índice 1
    if df.shape[1] < 2:
        return None
    s = df.iloc[:, 1]
...
def _leads_status_get_status_col_q(df: pd.DataFrame) -> pd.Series:
    # coluna Q = índice 16
    if df.shape[1] < 17:
        return pd.Series([], dtype="string")
    return df.iloc[:, 16].astype("string").fillna("").str.strip()

def _leads_status_count_validas(df: pd.DataFrame, data_base: dt.date) -> int:
    # coluna M = índice 12
    if df.shape[1] < 13:
        return 0
    cad = df.iloc[:, 12]
...
def _leads_status_store_upsert(day_key: str, status_counts: Dict[str, int], validas: int):
    store = safe_json_load(HIST_LEADS_STATUS_Q, default={})
    store[day_key] = {
        "status": {str(k): int(v) for k, v in (status_counts or {}).items()},
        "validas": int(validas or 0),
    }
    safe_json_save(HIST_LEADS_STATUS_Q, store)

def _leads_status_store_to_df() -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Retorna:
      - df_status: Data, Status, Quantidade, _date, Mes(YYYY-MM)
      - df_validas: Data, Validas, _date, Mes(YYYY-MM)
    """
    store = safe_json_load(HIST_LEADS_STATUS_Q, default={}) or {}
    rows = []
    rows_v = []
    for dkey, payload in store.items():
        try:
            d = dt.datetime.strptime(dkey, "%d/%m/%Y").date()
        except Exception:
            continue
        if d < HIST_START:
            continue

        status_map = (payload or {}).get("status", {}) or {}
        validas = int((payload or {}).get("validas", 0) or 0)

        for stt, qty in status_map.items():
            rows.append({"Data": dkey, "_date": d, "Mes": f"{d.year:04d}-{d.month:02d}", "Status": str(stt), "Quantidade": int(qty)})
        rows_v.append({"Data": dkey, "_date": d, "Mes": f"{d.year:04d}-{d.month:02d}", "Válidas": validas})

    df_status = pd.DataFrame(rows) if rows else pd.DataFrame()
    df_validas = pd.DataFrame(rows_v) if rows_v else pd.DataFrame()
    return df_status, df_validas

if up_leads_status:
    df_ls = read_excel_any(up_leads_status.getvalue())
    data_base = _leads_status_get_data_base_from_col_b(df_ls)
    if data_base is None:
        st.error("Não consegui identificar a DATA BASE na coluna B do arquivo.")
    else:
        s_status = _leads_status_get_status_col_q(df_ls)
        s_status = s_status[s_status.astype(str).str.strip() != ""]
        counts = s_status.value_counts().to_dict() if not s_status.empty else {}
        validas = _leads_status_count_validas(df_ls, data_base)

        day_key = data_base.strftime("%d/%m/%Y")
        _leads_status_store_upsert(day_key, counts, validas)

        st.success(f"Salvo no histórico: {day_key} | Válidas: {br_int(validas)}")

df_status, df_validas = _leads_status_store_to_df()

if df_status.empty:
    st.info("Importe um arquivo para iniciar o histórico do Status diário (coluna Q).")
else:
    # Filtro mês
    meses = sorted(df_status["Mes"].unique())
    meses_lbl = []
    for m in meses:
        try:
            y, mm = m.split("-")
    ...
    mes_sel_lbl = st.selectbox("Selecione o mês", meses_lbl, index=len(meses_lbl) - 1, key="ls_mes_sel")
    mes_sel = meses[meses_lbl.index(mes_sel_lbl)]

    # Pivot diário (status) + total
    dfm = df_status[df_status["Mes"] == mes_sel].copy()
    pivot = (
    ...
    if not df_validas.empty:
        dfv_m = df_validas[df_validas["Mes"] == mes_sel].copy()
        vmap = dfv_m.set_index("_date")["Válidas"].to_dict()
    ...
    # Tabela curta (bonita) — só Total/Válidas e deltas
    view = pd.DataFrame(index=pivot.index)
    view["Data"] = [d.strftime("%d/%m/%Y") for d in pivot.index]
    view["Total"] = pivot["Total"].apply(br_int)
    view["Δ Total"] = delta["Total"].apply(br_int)
    view["Válidas"] = pivot["Válidas"].apply(br_int)
    view["Δ Válidas"] = delta["Válidas"].apply(br_int)
    ...
    def _style_delta(val: str):
        ...
        return "color:#5b6b8c; font-weight:800;"

    st.markdown("### Comparativo diário (Δ vs dia anterior) — Leads Status")
    st.dataframe(
        view.style.applymap(_style_delta, subset=["Δ Total", "Δ Válidas"]),
        use_container_width=True,
        hide_index=True
    )
    ...
    # Métricas do mês
    c1, c2 = st.columns(2)
    c1.metric("Total no mês", br_int(int(pivot["Total"].sum())))
    c2.metric("Indicações válidas no mês (≤14 dias)", br_int(int(pivot["Válidas"].sum())))
