...
with tab_meta:
    ...
    def _match_group(broadcast_desc: str, group_name: str) -> bool:
        ...
        for k in keys:
            if k.upper() in txt:  # Simples: "AMERIC" in "MATHEUSAMERIC" → True
                return True
    ...
    def _compute_group_aggregates_from_raw_df(df5: pd.DataFrame) -> Dict[str, Dict[str, pd.DataFrame]]:
        ...
        for gname in _groups_definitions().keys():
            ...
            dfg = df[mask].copy()
            if dfg.empty:
                out[gname] = {"monthly": pd.DataFrame(columns=["Mes", "message_status", "qty"]),
    ...
    if meta_files:
        # Processar arquivos com controle inteligente (apenas para Meta)
    ...
    if not summary:
        ...
        with st.expander("Carteira — clique para abrir", expanded=False):
            ...
            st.markdown("**Regra:** se a palavra aparecer em qualquer lugar do texto, contabiliza. Ex: 'matheusameric' → VAREJO, 'i9hoje' → I9")
            ...
            if st.button("Processar", key="meta_groups_process_btn"):
                if not grp_files:
                    ...
                    # Processar arquivos com controle (apenas para grupos)
                    dfs, novos_metadados, qtd_novos, qtd_substituidos = process_meta_files_with_control(
                        grp_files,
                        META_GROUPS_CONTROL,
                        lambda name, raw: _read_meta_file(name, raw)
        ...
        # Observação única (não duplicada)
        st.info("Obs.: para 'Baixar analítico do dia' com linhas, precisaríamos guardar o arquivo cru (muito pesado). Aqui o histórico é consolidado (e persistente) por dia/status/mês e por grupo.")
...
with tab_leads_status:
    ...
    # ----- Função para Calcular Indicações Válidas (Regra dos 14 Dias) -----
    def _calcular_validas_14d(df: pd.DataFrame, data_base: dt.date) -> int:
        ...
        # Extrai a coluna M (índice 12) que contém a data/hora de cadastro
        col_cadastro = df.iloc[:, 12]
        
        # Converte para datetime, priorizando formato dia/mês/ano
        datas_cadastro = pd.to_datetime(col_cadastro, errors="coerce", dayfirst=True)
        
        # Filtra apenas linhas com datas válidas
        mascara_valida = datas_cadastro.notna()
        if not mascara_valida.any():
            return 0
            
        # Converte a data_base para datetime para comparação
        data_base_dt = pd.to_datetime(data_base)
        
        # Calcula a diferença em dias apenas para linhas válidas
        diferencas = (data_base_dt - datas_cadastro[mascara_valida]).dt.days
        
        # Conta quantas têm diferença entre 0 e 14 dias (não pode ser negativa)
        validas = ((diferencas >= 0) & (diferencas <= 14)).sum()
    ...
    with st.expander("📤 Importar arquivo(s) diário(s)", expanded=True):
        ...
        *   **Indicações Válidas (≤14 dias)**: Calculado se o arquivo tiver 13+ colunas (coluna M).
        """)
        ...
        if up_status_files:
            # Processar com controle (reaproveita a função da Meta)
            dfs, novos_metadados, qtd_novos, qtd_substituidos = process_meta_files_with_control(
                up_status_files,
                LEADS_CONTROL_PATH,
            ...
            if dfs:
                for i, df_status in enumerate(dfs):
                    ...
                    if s.empty:
                        ...
                        status_counts = s.value_counts().to_dict()
                        status_counts = {str(k): int(v) for k, v in status_counts.items()}

                    # 2. Calcular Indicações Válidas (regra dos 14 dias)
                    ...
                    # Se já existir um payload para o dia, atualiza/mescla os status
                    payload_existente = store_clean.get(day_key, {})
                    if isinstance(payload_existente, dict):
                        # Mescla os novos status com os antigos (substitui se houver conflito)
                ...
                # Salvar tudo de volta no storage
                _leads_status_save(store_clean)
                st.success("✅ Histórico atualizado com sucesso!")
                st.rerun()

    # ----- Exibição do Painel -----
    # Recarrega a store limpa para exibição
    store = _leads_status_load()
    store_clean = {k: v for k, v in store.items() if not k.startswith("_")}
    ...
    if not store_clean:
        ...
        if not rows:
            ...
            # --- Tabela Comparativa Diária (com Δ) ---
            df_mes = dfh[dfh["Mes"] == mes_sel].copy()

            if not df_mes.empty:
                st.markdown("### 📈 Comparativo Diário (Δ vs dia anterior)")

                # Pivot para ter uma linha por dia, colunas por status
                ...
                # --- REDUZIR NOMES DAS COLUNAS DE STATUS ---
                short_names = {}
                for col in status_cols:
                    # Reduzir nomes muito longos
                    if len(col) > 25:
                        # Tenta pegar as primeiras palavras
                        words = col.split()
                        if len(words) > 3:
                            short = ' '.join(words[:3]) + '...'
                        else:
                ...
                # Renomear colunas no pivot
                pivot = pivot.rename(columns=short_names)
                status_cols_short = [short_names.get(c, c) for c in status_cols]
                
                # Calcular Δ para cada status
                for col in status_cols_short:
                    pivot[f"Δ {col}"] = pivot[col].diff().fillna(0).astype(int)

                # Adicionar coluna de Total e Δ Total
                pivot["Total"] = pivot[status_cols_short].sum(axis=1)
                pivot["Δ Total"] = pivot["Total"].diff().fillna(0).astype(int)

                # Adicionar coluna de Indicações Válidas e Δ
                ...
                pivot_display["_date"] = pivot_display["_date"].dt.strftime("%d/%m/%Y")
                pivot_display = pivot_display.rename(columns={"_date": "Data base"})
                ...
                delta_cols = [c for c in pivot_display.columns if c.startswith("Δ ")]
                
                # Aplicar estilo
                styled_df = pivot_display.style
                
                if delta_cols:
                    styled_df = styled_df.apply(color_delta_series, subset=delta_cols)

                # Aplicar negrito à coluna Total e Indicações Válidas
                styled_df = styled_df.applymap(
                    lambda x: 'font-weight: 800;' if pd.notna(x) else '',
                    subset=["Total", "Indicações Válidas"]
                )
