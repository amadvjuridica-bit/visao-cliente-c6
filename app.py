...
LOGO_PATH = "LOGO CORRETA.png"  # precisa estar na raiz do repo
...
# =========================
COL_T  = "DT_CONTA_CRIADA"
COL_P  = "DT_FUNDACAO_EMPRESA"
COL_X  = "CHAVES_PIX_FORTE"
COL_Y  = "VL_SALDO_MEDIO_MENSALIZADO"
COL_V  = "STATUS_CC"
COL_AQ = "BANCO_DOMICILIO"
...
# COLUNAS (Leads)
# =========================
LEADS_CADASTRO = "DATA_HORA_CADASTRO"
...
def inject_css():
    st.markdown(
        ...
        <style>
            ...
            .am-card-title { font-size: 13px; opacity: .75; margin-bottom: 4px; }
            .am-card-value { font-size: 26px; font-weight: 700; color: #1b2440; margin: 0; }
            .am-card-sub { font-size: 12px; opacity: .70; margin-top: 6px; }
            .am-divider { height: 1px; background: #eef0f6; margin: 16px 0; }
        </style>
        """,
...
# =========================
# REGRAS — Qualificação / Nível / Receita
# =========================
...
def compute_qualificadas(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, int]:
    """
    Qualificada = BY == 1
    Nível = maior valor dos critérios (1..4)
    Receita = PAYOUT[nivel]
    """
    dfq = df[df[COL_BY] == 1].copy()
    dfq["Nível"] = dfq[COL_CRIT].apply(parse_max_level).astype(int)
    dfq = dfq[dfq["Nível"] >= 1].copy()  # só níveis 1..4 contam

    dfq["Pago por nível"] = dfq["Nível"].map(PAYOUT).fillna(0).astype(int)
    dfq["Receita"] = dfq["Pago por nível"].astype(int)

    # Tabela por nível: qtd, pago, total
    dist = (
        dfq["Nível"]
        .value_counts()
        .sort_index()
    ...
    dist["Pago por nível"] = dist["Nível"].map(PAYOUT).fillna(0).astype(int)
    dist["Total (nível)"] = dist["Quantidade"] * dist["Pago por nível"]
...
def contas_criadas(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, int]:
    s = safe_to_date(df[COL_T]).dropna()
    s = s[s >= START_DATE]
...
def status_counts(df: pd.DataFrame) -> pd.DataFrame:
    s = normalize_str(df[COL_V]).replace("", "SEM STATUS")
    out = (
        s.value_counts()
        .rename_axis("Status")
        .reset_index(name="Quantidade")
...
if up_leads:
    ...
    # histórico (guarda com Percentual_num para pintar, mas NÃO exibe)
    upsert_hist(HIST_LEADS_DAILY, "Dia", daily_leads.drop(columns=["Percentual"], errors="ignore"))
    upsert_hist(HIST_LEADS_MONTHLY, "Mês", monthly_leads.drop(columns=["Percentual"], errors="ignore"))
...
if latest_snap and latest_snap.get("metrics"):
    ...
    with r1:
        card("Contas abertas (no arquivo)", fmt_thousand(m["contas_abertas_total"]))
    with r2:
        card("Saldo total", br_money(m["saldo_total"]))
    with r3:
        card("Clientes com Pix", fmt_thousand(m["clientes_com_pix"]))
    with r4:
        card("Clientes sem Pix", fmt_thousand(m["clientes_sem_pix"]))

    r5, r6, r7, r8 = st.columns(4)
    with r5:
        card("Domicílio C6", fmt_thousand(m["domicilio_c6"]))
    with r6:
        card("Contas qualificadas", fmt_thousand(m["qualificadas"]))
    with r7:
...
with tabs[0]:
    ...
    if os.path.exists(HIST_OPEN_DAILY):
        ...
        with c1:
            st.markdown("**Diário**")
            hist_daily["Contas abertas"] = hist_daily["Contas abertas"].apply(fmt_thousand)
            st.dataframe(hist_daily, hide_index=True, use_container_width=True)
        with c2:
            st.markdown("**Mensal**")
            hist_month["Contas abertas"] = hist_month["Contas abertas"].apply(fmt_thousand)
            st.dataframe(hist_month, hide_index=True, use_container_width=True)
    else:
        st.info("Importe o arquivo Visão Cliente pelo menos uma vez para iniciar o histórico.")
...
with tabs[1]:
    ...
    if df_clientes is None:
        ...
        if fund.empty:
            ...
            show = show.drop(columns=["Dia"]).sort_values("Quantidade", ascending=False)
            show["Quantidade"] = show["Quantidade"].apply(fmt_thousand)

            total_dia = int(fund[fund["Dia"] == sel_date]["Quantidade"].sum())
            card("Aberturas no dia selecionado", fmt_thousand(total_dia), sub=f"Dia: {sel}")
...
with tabs[2]:
    ...
    if df_clientes is None:
        ...
        with st1:
            card("Clientes com Pix", fmt_thousand(com_pix))
            pix_by_type["Quantidade"] = pix_by_type["Quantidade"].apply(fmt_thousand)
            st.dataframe(pix_by_type, hide_index=True, use_container_width=True)
        with st2:
            card("Clientes sem Pix", fmt_thousand(sem_pix))
            stt = status_counts(df_clientes)
            stt["Quantidade"] = stt["Quantidade"].apply(fmt_thousand)
            st.dataframe(stt, hide_index=True, use_container_width=True)
...
with tabs[3]:
    ...
    if df_clientes is None:
        ...
        a, b = st.columns([1, 2])
        with a:
            card("Total qualificadas", fmt_thousand(dfq.shape[0]))
            card("Receita estimada", br_money(receita_total))
        with b:
            st.markdown("**Receita por nível**")
            show = nivel_dist.copy()
            show["Quantidade"] = show["Quantidade"].apply(fmt_thousand)
            show["Pago por nível"] = show["Pago por nível"].apply(lambda x: br_money(float(x)))
            show["Total (nível)"] = show["Total (nível)"].apply(lambda x: br_money(float(x)))
            st.dataframe(show, hide_index=True, use_container_width=True)

        st.markdown("#### Lista (auditoria)")
        cols_show = []
        ...
        audit["Nível"] = dfq["Nível"].apply(fmt_thousand)
        audit["Valor pago"] = dfq["Pago por nível"].map(lambda x: br_money(float(x)))
...
with tabs[4]:
    ...
    if not os.path.exists(HIST_LEADS_DAILY):
        ...
        # reconstrói percentual (para pintar), mas NÃO exibe Percentual_num
        aux_d = hist_d.copy()
        ...
        with c1:
            ...
            show_d["Cadastradas"] = show_d["Cadastradas"].apply(fmt_thousand)
            show_d["Abertas"] = show_d["Abertas"].apply(fmt_thousand)

            styled = style_percentual(show_d, threshold=20.0)
            styled = styled.hide(axis="index")

            # remove Percentual_num da visualização
            styled = styled.format(na_rep="")
            st.dataframe(
                styled,
                use_container_width=True,
                column_config={"Percentual_num": None}
            )

        with c2:
            st.markdown("**Mensal**")
            show_m = aux_m[["Mês", "Cadastradas", "Abertas", "Percentual", "Percentual_num"]].copy()
            show_m["Cadastradas"] = show_m["Cadastradas"].apply(fmt_thousand)
            show_m["Abertas"] = show_m["Abertas"].apply(fmt_thousand)

            styled2 = style_percentual(show_m, threshold=20.0)
            styled2 = styled2.hide(axis="index")
            styled2 = styled2.format(na_rep="")
            st.dataframe(
                styled2,
                use_container_width=True,
                column_config={"Percentual_num": None}
